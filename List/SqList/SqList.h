...
//【算法--交集】
template<class DT>
void AjiaoB(SqList<DT>& L1, SqList<DT>& L2)
{
	DT e;
	int i = 1;
	while(i<=ListLength(L2))	
	{
		GetElem_i(L2, i, e);
		{
			if (LocateElem_e(L1, e)==0)
			{
				DeleElem_i(L2, i);
			}
			else
...
//【算法--并集】
template<class DT>
void AbingB(SqList<DT>& L1, SqList<DT>& L2)
{
	DT e;
	for (int i = 1; i <= L1.length; i++)
	{
		GetElem_i(L1, i, e);
		if (LocateElem_e(L2, e) == 0)
		{
			InsertElem_i(L2, L2.length + 1, e);
		}
	}
	cout << "两顺序表的并集是：" << endl;
	DispList(L2);
